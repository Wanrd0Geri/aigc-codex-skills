...
1. Establish total-time evidence from exact dialogue speaking time, indivisible action phases, current-user or source timing, and controlled duration comparisons. Treat the feasibility estimates in `single-segment-quality-control.md` as planning ranges; rendered timestamps remain current-user/source-backed or unresolved. A failed, rigid, or discontinuous result does not authorize inferred shot-body time ranges: first repair the current state, route, contact, transition, and endpoint in causal prose. A controlled comparison may support an internal timeline, but render that timeline only when the current user explicitly adopts it.
...
Treat the observed error and correction discussion as diagnosis evidence, not as content for Seedance. Pass every touched older correction to the semantic disposition in `change-impact-and-delivery.md`; this file never deletes an accepted lock merely because a newer message omits or rephrases it. After identifying the smallest repair, rewrite that field as a standalone current visible state, action, spatial relationship, or endpoint that remains intelligible without the prior prompt or failed result. Express the intended current result directly. Retain a short local negative only when the target cannot be resolved positively and the active rules authorize it. Do not preserve diagnostic timing scaffolds after they have served attribution; recompile the affected shot as one current causal process unless the current user or authoritative source explicitly locks the internal time.
...
| Two subjects repeatedly receive equal visual weight although one should dominate | Keep one confirmed viewer priority, express it once as `画面重心`, and reduce the other subject to the minimum visible relationship needed for scale, causality, or continuity | Remove a locked subject or invent an exact screen percentage |
| Character looks pasted into the environment or receives light from the wrong source | Run `LightCompositeSpec`: retire baked light from appearance-only references, preserve the scene's world-space source anchor, and connect subject, contact/ground, one visible nearby/depth receiver, and camera exposure | Add `光影统一` or a generic quality/compositing suffix |
...
| A material reaction stays on one expression, changes only for an instant, or returns to its opening mask before the cut | Re-run the performance-capacity gate; choose a crop-readable start/end contrast, remove competing mutable cues, and hold the reached endpoint into the cut and next boundary when material | Add more emotion adjectives, stack facial micro-actions, or force a universal reaction duration |
| A silent listener opens the mouth and reads as speaking or gasping | Remove the unsupported mouth-opening cue and transfer the visible change to attention, head/shoulder set, lip-line or jaw tension, posture, distance, or object contact | Add a global lip-sync prohibition or rewrite unrelated dialogue |
| Characters rotate toward each other at a cut although no turn was shown | Restate the preserved world-facing and position as the new shot's current state, then place the camera around that blocking | Stack negative facing commands, rotate the subjects to expose a face, or require a Diagram before direct prompt repair is tested |
| An approach repeatedly completes as contact although a visible non-contact gap is locked | After the attribution pass identifies the shared approach/endpoint field and retires any exhausted Diagram hypothesis, test one case-specific `prompt_repair`: replace the contact-completing action with an observable separated endpoint that stops or withdraws before the target, and require the terminal frame to keep a clearly visible gap as the discriminating criterion. If the exact near-gap cannot be verified in the current crop, isolate it in a verifiable crop or use readable accepted boundary evidence supported by the active workflow; if the repair misses its criterion, re-open attribution | Stack `绝不接触` variants, assume a staging Diagram guarantees the fine gap, keep rerunning the same contact-inducing action, or claim a provider-wide capability boundary from this case |
