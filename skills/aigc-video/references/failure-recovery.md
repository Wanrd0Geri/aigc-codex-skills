...
Load this file only when a Seedance-family result has failed, the user supplies paired results, or an existing supported-family prompt is known to be unstable. Do not preload these controls on a first attempt or generalize its heuristics to other video models.
...
Use only attributes already authorized by the user, source, project, or assigned reference role. Do not introduce a new material system, symbol language, geometry, prop, action, camera choice, or style axis to make a correction distinct.
...
| Reference role leaks | Restore the assigned role and affected field | Print the full internal forbidden-role map |
...
| A revealed figure freezes after formation or resembles a held reference pose | Narrow any over-broad pose/composition reference role, then connect formation directly to the next visible action and keep one motion carrier active through the reveal | Add several unrelated camera moves or assume every pause comes from the prompt without a controlled comparison |
