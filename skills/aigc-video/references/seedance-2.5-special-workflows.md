...
When the source video owns the whole clip’s timing, inherit its duration, shot order, cuts, and rhythm under `VIDEO-TIME-01` in `seedance-2-rules.md`. Bind those dimensions once and use ordered prompt-local `镜头N：` entries without ranges even when exact cuts are readable; do not ask for or separately write total duration. Keep measured timecodes internal and render only explicitly required numeric ranges or synchronization cues. When the video supplies only action/camera, use the target-duration timeline instead. Never reuse project/source shot ids as headings. Describe articulated limbs, wings, tails, mechanisms, or camera motion only when they are present in the current visible set; describe the complete supported process through preparation, movement, contact, and endpoint instead of naming only the result. Express those phases as one causal process inside the shot and do not derive internal second ranges from coarse-model keyframes or uneven motion. Do not add a new shot, transition, or editorial beat.
