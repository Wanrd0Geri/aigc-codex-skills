...
- A final Seedance 2.5 previsualization prompt uses the active adapter's canonical timeline. If total duration is missing, ask for it — except when a supplied previsualization/coarse/white-model video owns the whole clip's timing and cuts; then inherit them and use untimed shot headings under `VIDEO-TIME-01` without asking. A video borrowed only for action or camera does not grant this exception. Once supplied or inherited, do not delete or combine locked shots to make the allocation easier.
