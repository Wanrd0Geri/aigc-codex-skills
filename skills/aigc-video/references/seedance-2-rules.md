...
Preserve literal anchors such as `@图1`、`@视频1`、`@音频1` and `@文件名.png`. If the user writes `图1` or `参考图1`, normalize it to `@图1` only when the attachment mapping is established; otherwise ask rather than inventing an anchor.
